...
func QueryShopById(id int) (result *dto.Result){
	result = &dto.Result{}
	...
	//逻辑过期解决缓存击穿
	shop,err := QueryShopWithLogicalExpire(id)
	if err!= nil{
...
//逻辑时间解决缓击穿（已经提前预热）
func QueryShopWithLogicalExpire(id int) (model.Shop,error){
	key := utils.CACHE_SHOP_KEY+strconv.Itoa(id)
	cacheShop,err := redis.RDB.Get(redis.RCtx,key).Result()
	if (err == goredis.Nil || cacheShop == ""){
		return model.Shop{},errors.New("商铺信息不存在！")
...
func SaveShop2Redis(id uint64,expireTime int64){
	...
	shopJson,err := json.Marshal(redisData)
	if err!= nil{
		return
	}
	redis.RDB.Set(redis.RCtx,utils.CACHE_SHOP_KEY+strconv.FormatUint(id,10),shopJson,-1).Result()
}